...
func writeGoStruct(targetStruct *ParsedDirectory, goStructElements map[string]*ParsedDirectory, generatedUnions map[string]bool, ignoreShadowSchemaPaths bool, goOpts GoOpts, generateJSONSchema bool) (GoStructCodeSnippet, []error) {
	...
	for _, fName := range targetStruct.OrderedFieldNames() {
		...
		case ListNode:
			...
			fieldType, multiKeyListKey, listMethods, listErr := yangListFieldToGoType(field, fieldName, targetStruct, goStructElements, goOpts.GenerateSwaggerCompliantCode)
			if listErr != nil {
		...
		if field.ExtensionTags != "" {
			tagBuf.WriteString(field.ExtensionTags)
		}
		...
		if field.SwaggerTags != "" {
			tagBuf.WriteString(field.SwaggerTags)
		}
		...
		if goOpts.AddDescriptionsAsComments {
			if goOpts.RetainOnlyDeprecationComments {
				if strings.HasPrefix(field.YANGDetails.Description, deprecationCommentPrefix) {
					fieldDef.Description = field.YANGDetails.Description
				}
			} else {
				fieldDef.Description = field.YANGDetails.Description
			}
...
func yangListFieldToGoType(listField *NodeDetails, listFieldName string, parent *ParsedDirectory, goStructElements map[string]*ParsedDirectory, swaggerCompliance bool) (string, *generatedGoMultiKeyListStruct, *generatedGoListMethod, error) {
